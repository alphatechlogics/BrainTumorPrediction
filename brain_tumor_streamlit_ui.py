...
def load_model(device: torch.device) -> torch.nn.Module:
    ...
    model = efficientnet_v2_s(
        weights=EfficientNet_V2_S_Weights.DEFAULT).to(device)
    num_ftrs = model.classifier[1].in_features
    model.classifier[1] = torch.nn.Linear(num_ftrs, len(CLASSES))
    model.load_state_dict(torch.load(
        'fine_tuned_efficientnet_v2_s.pth', map_location=device))
    model.to(device)
    model.eval()
    return model
...
def main():
    ...
    with col2:
        st.image(contoured_image, caption='Contoured Image',
                 use_container_width=True)

    if not is_mri:
        st.warning(
            "The uploaded image does not seem to be a valid MRI image. Proceeding with caution.")
    else:
    ...
    if st.button("Predict and Classify"):
        ...
        prediction, confidence, probabilities = predict_image(
            st.session_state.image, model, device, transform)

        if prediction == "not_valid_mri":
            st.error(
                f"The image is classified as not a valid MRI image. Confidence: {confidence:.2f}")
            ...
            st.success(
                f"Prediction: {prediction.capitalize()} tumor detected. Please consult a medical professional.")
